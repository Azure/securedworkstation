...
param (
    ...
    #Adding Search String to CA Export
    [String]$Prefix = "PAW"
...
$Resource = "deviceManagement/deviceCompliancePolicies?`$expand=Assignments,scheduledActionsForRule(`$expand=scheduledActionConfigurations)"
    
    try {

        $uri = "https://graph.microsoft.com/$graphApiVersion/$($Resource)"
        (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value | Where-Object { ($_.'@odata.type').contains("windows10CompliancePolicy") -and ($_.'displayName').contains($Prefix) }
...
$DCPPolicies = Get-DeviceCompliancePolicy

Write-Output $DCPPolicies

foreach ($DCP in $DCPPolicies)

{
    ...
    $DCP.assignments.target
     ...
     #$JSON_DATA = $DCP | ConvertTo-Json -depth 10

     $DCP.PSObject.Properties.Remove('scheduledActionsForRule@odata.context')

     $DCP.PSObject.Properties.Remove('assignments@odata.context')
